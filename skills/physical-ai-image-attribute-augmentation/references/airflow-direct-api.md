...
This is the primary path for triggering and monitoring Image Attribute Augmentation runs. The
controller also deploys the SDG webserver API, which `scripts/workflow.py` targets, but that path
authenticates via NGC and is not required for normal operation.

## Preflight (direct path)

`workflow.py preflight` requires `WEBSERVER_ENDPOINT`. For the Airflow path, run these checks
instead:
...
DAG_ID="image_attribute_augmentation_dag_k8s"
...
RESPONSE="$RESPONSE" python3 -c "
import json, os
watched = ('k8s_gpu_1','default_pool','external_image_edit_service_pool','iaa_internal_image_edit_service_pool')
...
Every IAA task manifest sets `kubernetes_conn_id: kubernetes_remote`, so this connection must
resolve or all tasks fail at runtime. **Do not verify it through the REST API.** This deployment
injects connections as `AIRFLOW_CONN_*` environment variables sourced from Kubernetes secrets
(see `deploy/values.yaml`), and env-var connections are never written to the Airflow metadata
database. `GET /api/v2/connections` therefore returns `total_entries: 0` on a perfectly healthy
cluster, and `GET /api/v2/connections/kubernetes_remote` returns 404. Neither means anything is
wrong.
...
default `Param` value. Without it the DAG runs with `ImageAttributeAugmentationDagPayloadConfig()`
defaults, which point at shared sample data and endpoints that are not yours — always pass an
explicit payload for user runs.
...
PAYLOAD=$(cat /tmp/iaa-payload.json)
curl -s -X POST "$AIRFLOW_URL/api/v2/dags/$DAG_ID/dagRuns" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
