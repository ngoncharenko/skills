...
|---|---|---|
| Is this skill safe to run? | Scan report over the Tier 1 staged scope | SkillSpector, inside SkillEvaluator Tier 1 |
...
- **Tier 1 — Validation.** Schema, license, PII, Unicode safety, and the SkillSpector security scan over a staged subset of the skill (artifact and evaluation trees excluded). Deterministic, and it can fail a skill outright.
...
3. Run SkillEvaluator against the complete skill directory. Tier 1 includes the SkillSpector scan on its staged scope (see the [release checklist](/skills/release-checklist#tier-1-skillspector-scope)).
4. Fix high-risk findings in that scope, or record why a finding is accepted.
